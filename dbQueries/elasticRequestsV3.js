const elastic = {
    ...
    async searchPhrase (connection, data) {
        const { options, settings } = data;
        if (!settings.sort) settings.sort = [];
        const result = await connection.client.search({
    ...
    async searchUnique (connection, { options, settings }) {
        return await connection.client.search({
            index: connection.index,
            body: {
                query: {
                    match: {
    ...
    async searchTerms (connection, data) {
    ...
    async getTerm (connection, termId) {
        return await connection.client.get({
            index: connection.index,
            type: "_doc",
            id: termId
        });
    ...
    async searchLists (connection, data) {
        ...
        if (type) {
            request.body.query.bool.must.push({ match: { type } });
        }
        if (store) {
            request.body.query.bool.must.push({ match: { store } });
        }
        if (!empties) {
            request.body.query.bool.must.push({ exists: { "field": "items.itemId" } });
        }
        ...
        const res = await connection.client.search(request);
        try {
            return res;
        } catch (err) {
            return new Error(err);
